...
const UnverifiedUser = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            setUsers(usersData)
            // Filter only unverified users (verified === false)
            const unverified = usersData.filter(user => user.verified === false)
            ...
            <div className="min-h-screen bg-white dark:bg-black flex items-center justify-center">
                <div className="text-center">
                    <div className="relative">
                        ...
                        <ShieldAlert className="w-6 h-6 text-yellow-500 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
                    </div>
                    <p className="mt-4 text-gray-600 dark:text-gray-400">Loading unverified users...</p>
                </div>
            </div>
        )
    }
    ...
    return (
        <div className="min-h-screen bg-white dark:bg-black">
            {/* Hero Section */}
            <div className="relative overflow-hidden bg-gradient-to-br from-yellow-50 via-white to-white dark:from-yellow-950 dark:via-black dark:to-black pt-12 pb-20">
                <div className="absolute inset-0 opacity-30">
                    <div className="absolute top-20 left-10 w-72 h-72 bg-yellow-400 rounded-full blur-[100px] dark:bg-yellow-600"></div>
                    <div className="absolute bottom-20 right-10 w-96 h-96 bg-yellow-300 rounded-full blur-[120px] dark:bg-yellow-500"></div>
                </div>
                
                <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center">
                        <div className="inline-flex items-center gap-2 px-4 py-2 bg-yellow-100 dark:bg-yellow-500/10 border border-yellow-300 dark:border-yellow-500/20 rounded-full mb-6">
                            <ShieldAlert className="w-4 h-4 text-yellow-600 dark:text-yellow-500" />
                            <span className="text-sm text-yellow-600 dark:text-yellow-400">Unverified Users Management</span>
                        </div>
                        ...
                        <p className="text-gray-600 dark:text-gray-400 text-lg max-w-2xl mx-auto">
                            Manage and verify users who haven't completed email verification
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-sm rounded-2xl p-5 border border-gray-200 dark:border-zinc-800 hover:border-yellow-400 dark:hover:border-yellow-500/30 transition-all group shadow-sm hover:shadow-md cursor-pointer">
                        ...
                        <p className="text-2xl font-bold text-gray-900 dark:text-white">{unverifiedUsers.length}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-500">Total Unverified Users</p>
                    </div>
                    ...
                    <div className="bg-white dark:bg-zinc-900/50 backdrop-blur-sm rounded-2xl p-5 border border-gray-200 dark:border-zinc-800 hover:border-yellow-400 dark:hover:border-yellow-500/30 transition-all group shadow-sm hover:shadow-md cursor-pointer">
                        ...
                        <p className="text-2xl font-bold text-gray-900 dark:text-white">
                            {((unverifiedUsers.length / users.length) * 100).toFixed(1) || 0}%
                        </p>
                        <p className="text-sm text-gray-500 dark:text-gray-500">Unverified Percentage</p>
                    </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-4">
                        <div className="flex-1 relative">
                            ...
                            <input
                                ...
                                className="w-full pl-11 pr-4 py-3 bg-gray-50 dark:bg-black border border-gray-200 dark:border-zinc-800 rounded-xl text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:border-yellow-500 focus:ring-2 focus:ring-yellow-500/20 transition"
                            />
                        </div>
                        <div className="flex gap-3">
                            <select
                                ...
                                className="px-4 py-3 bg-gray-50 dark:bg-black border border-gray-200 dark:border-zinc-800 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:border-yellow-500 cursor-pointer"
                            >
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="relative">
                                                        <div 
                                                            ...
                                                            className="w-10 h-10 rounded-xl bg-gradient-to-br from-yellow-500 to-yellow-700 flex items-center justify-center text-white font-bold cursor-pointer">
                                                        ...
                                                        <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white dark:border-black bg-yellow-500"></div>
                                                    </div>
                                                    <div>
                                                        ...
                                                        <button 
                                                            ...
                                                            className="ml-1 hover:text-yellow-600 dark:hover:text-yellow-400 transition"
                                                        >
                                    ...
                                    <tr>
                                        <td colSpan="5" className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                            <div className="flex flex-col items-center gap-2">
                                                ...
                                                <p className="text-sm text-gray-400">All users have completed email verification</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                    ...
                    {totalPages > 1 && (
                        <div className="flex justify-between items-center px-6 py-4 border-t border-gray-100 dark:border-zinc-800">
                            <p className="text-sm text-gray-500 dark:text-gray-500">
                                Showing {indexOfFirstItem + 1} to {Math.min(indexOfLastItem, filteredUsers.length)} of {filteredUsers.length} unverified users
                            </p>
                            <div className="flex gap-2">
                                <button 
                                    onClick={() => setCurrentPage(p => Math.max(1, p - 1))} 
                                    disabled={currentPage === 1} 
                                    className="px-3 py-1 bg-white dark:bg-zinc-800 rounded-lg text-gray-600 dark:text-gray-400 disabled:opacity-50 hover:bg-yellow-600 hover:text-white transition cursor-pointer"
                                >
                                    Previous
                                </button>
                                <span className="px-3 py-1 bg-yellow-600 text-white rounded-lg">{currentPage}</span>
                                <button 
                ...
                {/* Info Alert when no unverified users */}
                {unverifiedUsers.length === 0 && !loading && (
                    <div className="bg-green-50 dark:bg-green-900/20 rounded-2xl p-6 mb-8 border border-green-200 dark:border-green-800 text-center">
                        ...
                        <p className="text-sm text-green-600 dark:text-green-500 mt-1">
                            Great job! Every user has completed email verification.
                        </p>
                    </div>
                )}
            </div>
